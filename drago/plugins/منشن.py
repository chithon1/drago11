...
@dragoiq.ar_cmd(pattern="منشن(?:\s|$)([\s\S]*)")
async def menall(event):
    ...
    try:
        ...
        pass
@dragoiq.ar_cmd(pattern="الغاء منشن")
...
@dragoiq.ar_cmd(pattern="تاك(?:\s|$)([\s\S]*)")
async def Ahmed(event):
    ...
    participants = []
    async for member in dragoiq.iter_participants(chat):
    ...
    for i, member in enumerate(participants, start=1):
        ...
        if i % 99 == 0 or i == total_participants:
            ...
            try:
                await dragoiq.send_message(event.chat_id, final_message, reply_to=event.reply_to_msg_id)
    ...
    mention_in_progress = False
    await event.delete()
@dragoiq.ar_cmd(pattern="الغاء تاك(?:\s|$)([\s\S]*)")
async def Ahmed(event):
    global mention_in_progress
    if mention_in_progress:
        await event.edit("**⌁︙ تم الغاء عملية التاك بنجاح ✅**")
