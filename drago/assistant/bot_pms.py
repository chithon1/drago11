...
@dragoiq.bot_cmd(incoming=True, func=lambda e: e.is_private)
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await dragoiq.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await dragoiq.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await dragoiq.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await dragoiq.tgbot.get_entity(BOTLOG_CHATID)
            await dragoiq.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await dragoiq.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@dragoiq.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await dragoiq.get_entity(user_id)
    except Exception as e:
...
@dragoiq.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@dragoiq.bot_cmd(incoming=True, func=lambda e: e.is_private)
@dragoiq.bot_cmd(edited=True, func=lambda e: e.is_private)
