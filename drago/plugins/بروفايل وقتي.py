...
digitalpic_path = os.path.join(os.getcwd(), "drago", "digital_pic.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        dra = gvarstatus("DEFAULT_PIC") or "drago/helpers/styles/PaybAck.ttf"
        fnt = ImageFont.truetype(dra, 65)
        drawn_text.text((200, 200), current_time, font=fnt, fill=colo)
        img.save(autophoto_path)
        file = await dragoiq.upload_file(autophoto_path)
        try:
            if i > 0:
                await dragoiq(
                    functions.photos.DeletePhotosRequest(
                        await dragoiq.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await dragoiq(functions.photos.UploadProfilePhotoRequest(file))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await dragoiq(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@dragoiq.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
