...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await dragoiq(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        try:
            await dragoiq(functions.channels.EditTitleRequest(
                channel=await dragoiq.get_entity(event.chat_id),
        ...
        try:
            ...
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await dragoiq.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await dragoiq.upload_file(profile_pic)
        try:
...
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in dragoiq.iter_profile_photos(event.chat_id, limit=1) :
                    await dragoiq(
