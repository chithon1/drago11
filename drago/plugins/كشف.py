...
async def fetch_info(replied_user, event):
    ...
    caption += f"<b> {drago_EM}╎المعـرف  ⇠ </b> {username}\n"
    caption += f"<b> {drago_EM}╎الايـدي   ⇠ </b> <code>{user_id}</code>\n"
    caption += f"<b> {drago_EM}╎الرتبـــه  ⇠ {rotbat} </b>\n"
    caption += f"<b> {drago_EM}╎الصـور   ⇠ </b> {replied_user_profile_photos_count}\n"
    caption += f"<b> {drago_EM}╎الحساب ⇠ </b> "
    caption += f'<a href="tg://user?id={user_id}">{first_name}</a>'
    caption += f"\n<b> {drago_EM}╎البايـو    ⇠ </b> {user_bio} \n"
    if FFlXlX == True or user_id in FFlXlX:
    caption += f"<b> {drago_EM}الحسـاب ⇠  بـريميـوم\n"
