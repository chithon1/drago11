...
def paginate_help(
    ...
    if plugins:
        if len(pairs) > number_of_rows:
            ...
            pairs = pairs + [(Button.inline("القائمة الرئيسية", data="mainmenu"),)]
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("dragoiq"):
            ...
            builder = event.builder
            troll = os.path.join("./drago", "troll.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./drago", "hide.txt")
            try:
...
@dragoiq.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@dragoiq.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@dragoiq.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@dragoiq.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
