...
@dragoiq.ar_cmd(pattern="احجي(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    dragoevent = await edit_or_reply(event, "`يجري تنزيل الملف...`")
    oggfi = await event.client.download_media(reply, Config.TEMP_DIR)
    await dragoevent.edit("`يجري تحويل الكلام الى نص....`")
    r = sr.Recognizer()
    ...
    await dragoevent.edit(string_to_show)
...
@dragoiq.ar_cmd(pattern="استخرج(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    pic_file = await dragoiq.download_media(reply, Config.TMP_DOWNLOAD_DIRECTORY)
    if not pic_file:
