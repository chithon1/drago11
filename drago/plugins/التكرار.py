...
async def spam_function(event, drago, dragoiq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(dragoiq[0])
    if len(dragoiq) == 2:
        spam_message = str(dragoiq[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await drago.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and drago.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            drago = await event.client.send_file(
                event.chat_id, drago, caption=drago.text
            )
            await _catutils.unsavegif(event, drago)
        ...
        if BOTLOG:
            ...
            drago = await event.client.send_file(BOTLOG_CHATID, drago)
            await _catutils.unsavegif(event, drago)
        return
    elif event.reply_to_msg_id and drago.text:
        spam_message = drago.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@dragoiq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    drago = await event.get_reply_message()
    dragoiq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(dragoiq[0])
    except Exception:
    ...
    await spam_function(event, drago, dragoiq, sleeptimem, sleeptimet)

@dragoiq.on(admin_cmd(pattern="(مكرر|مؤقت|موقت)"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, dragoiq, sleeptimem, sleeptimet, DelaySpam=True)


@dragoiq.ar_cmd(pattern="تكرار الملصق$")
...
@dragoiq.ar_cmd(pattern="ايقاف التكرار ?(.*)")
