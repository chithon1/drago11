...
class DragoClient(TelegramClient):
    def ar_cmd(
        self: TelegramClient,
        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            ...
            if pattern is not None:
                ...
                if edited:
                    Dragoiq.add_event_handler(func, events.MessageEdited(**kwargs))
                Dragoiq.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                Dragoiq.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                Dragoiq.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
DragoClient.fast_download_file = download_file
DragoClient.fast_upload_file = upload_file
DragoClient.reload = restart_script
DragoClient.get_msg_link = get_message_link
DragoClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    DragoClient.send_message = send_message
    DragoClient.send_file = send_file
    DragoClient.edit_message = edit_message
