...
async def setup_bot():
    """
    To set up bot for drago
    """
    try:
        await dragoiq.connect()
        config = await dragoiq(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == dragoiq.session.server_address:
                ...
                break
        bot_details = await dragoiq.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        #await dragoiq.start(bot_token=Config.TG_BOT_USERNAME)
        #app = web.AppRunner(await web_server())
        #await app.setup()
        #bind_address = "0.0.0.0"
        #redaport = Config.PORT
        #await web.TCPSite(app, bind_address, redaport).start()
        dragoiq.me = await dragoiq.get_me()
        dragoiq.uid = dragoiq.tgbot.uid = utils.get_peer_id(dragoiq.me)
...
async def startupmessage():
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌁︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت دراكو تمت بنجاح**"
            )
...
async def mybot():
    DRAGO_USER = dragoiq.me.first_name
    The_noon = dragoiq.uid
    dragoiq_ment = f"[{DRAGO_USER}](tg://user?id={The_noon})"
    f"ـ {dragoiq_ment}"
    f"⪼ هذا هو بوت خاص بـ {dragoiq_ment} يمكنك التواصل معه هنا"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await dragoiq.send_message("@BotFather", perf)
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"drago/{folder}/*.py"
        plugin_path = f"drago/{folder}"
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await dragoiq.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await dragoiq.get_entity(BOTLOG_CHATID)
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await dragoiq.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if DRAGOBRANCH := branch:
        repourl = os.path.join(DRAGOREPO, f"tree/{DRAGOBRANCH}")
        gcmd = f"git clone -b {DRAGOBRANCH} {DRAGOREPO} {cfolder}"
        errtext = f"لا يوحد فرع بأسم `{DRAGOBRANCH}` في الريبو الخارجي {DRAGOREPO}. تاكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = DRAGOREPO
        gcmd = f"git clone {DRAGOREPO} {cfolder}"
        errtext = f"الرابط ({DRAGOREPO}) الذي وضعته لفار `EXTERNAL_REPO` غير صحيح عليك وضع رابط صحيح"
    response = urllib.request.urlopen(repourl)
    if response.code != 200:
        LOGS.error(errtext)
        return await dragoiq.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await dragoiq.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="drago", extfolder=cfolder)
