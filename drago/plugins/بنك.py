...
@dragoiq.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@dragoiq.ar_cmd(pattern="توب الفلوس(.*)")
...
@dragoiq.ar_cmd(pattern="مسح حسابي(.*)")
...
@dragoiq.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@dragoiq.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@dragoiq.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب الرافدين او دراكو الاسلامي
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    drago = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await dragoiq.send_file(
                message.chat_id,
                "https://telegra.ph/file/76af89af015c10a9128ac.jpg",
                caption=f"سرق [{drago}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@dragoiq.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "دراكو الاسلامي":
        bankn = "مصرف دراكو الاسلامي"
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "دراكو الاسلامي":
